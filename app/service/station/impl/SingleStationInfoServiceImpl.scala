...
class SingleStationInfoServiceImpl @Inject()(measService: MeasurementsService) extends SingleStationInfoService {
  ...
  private def jsonFromStationMeasurement(stationMeas: StationMeasurement) = {
    JsObject(Seq(
      ...
      "measurements" -> measService.getJsonSeqFromMeasurement(stationMeas.measurements)
    ))
  }
